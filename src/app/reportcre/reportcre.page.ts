...
import { first, buffer,map, tap,shareReplay } from "rxjs/operators";
...
@Component({
  selector: 'app-reportcre',
  templateUrl: './reportcre.page.html',
  styleUrls: ['./reportcre.page.scss'],
})
export class ReportcrePage implements OnInit {
  ...
  public foodList: UserRep[];
    ...
    public foodSource$ = this.firestore.collection<UserRep>('users').get().pipe(
      ...
      map(docs =>docs.map(old => {
        const newDoc = old;
        if(!newDoc.nivelCre) {
          newDoc.nivelCre = 'No definido'       
        }
        if(!newDoc.resultCre){
          newDoc.resultCre = 'No definido'
        }
        return newDoc;
      ...
      this.foodList = this.foodList.filter(currentFood => {
        ...
        if (currentFood.name && searchTerm) {            
          return (currentFood.name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 ||
            ...
            currentFood.nivelCre.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 );
        }
      });
    }
    ...
    generatePDF() {
      ...
      this.foodList.forEach(e=>{
        html += '<tr> <td>' + e.name + '</td><td>' + e.apellidos + '</td><td>' + e.cedula + '</td><td>' + e.nivelCre + '</td><td>' + e.resultCre + '</td></tr>'
      })
