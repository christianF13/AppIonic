...
@Component({
  ...
  templateUrl: './tcomunicacion.page.html',
  styleUrls: ['./tcomunicacion.page.scss'],
})
export class TcomunicacionPage implements OnInit {
  ...
  public AreaC: string="Comunicación y Colaboración";
  ...
  ngOnInit() {
    ...
    this.SerPreg.getPreguntasCom().subscribe(preguntas => {
 ...
 getResult(opciones:number[]){
    ...
    this.SerPreg.getPreguntasCom().subscribe(
      (data: any) => {
 ...
 async nivel(){
   ...
   await this.authservice.uploadResultC(this.SerPreg.correctAnswerCount,this.SerPreg.nivele, this.SerPreg.area);
