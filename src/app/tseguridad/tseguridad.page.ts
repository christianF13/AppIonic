...
@Component({
  selector: 'app-tseguridad',
  templateUrl: './tseguridad.page.html',
  styleUrls: ['./tseguridad.page.scss'],
})
export class TseguridadPage implements OnInit {
  ...
  public AreaSegu: string="Seguridad";
  ...
  ngOnInit() {
    ...
    this.SerPreg.getPreguntasSegu().subscribe(preguntas => {
  ...
  getResult(opciones:number[]){
    ...
    this.SerPreg.getPreguntasSegu().subscribe(
      (data: any) => {
 ...
 async nivel(){
    this.SerPreg.area = this.AreaSegu;
   ...
   await this.authservice.uploadResultSegu(this.SerPreg.correctAnswerCount,this.SerPreg.nivele,this.SerPreg.area);
