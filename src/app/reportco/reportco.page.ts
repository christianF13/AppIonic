...
@Component({
  selector: 'app-reportco',
  templateUrl: './reportco.page.html',
  styleUrls: ['./reportco.page.scss'],
})
export class ReportcoPage implements OnInit {
  ...
  public foodList: UserRep[];
    ...
    public foodSource$ = this.firestore.collection<UserRep>('users').get().pipe(
      ...
      map(docs =>docs.map(old => {
        const newDoc = old;
        if(!newDoc.nivelCo) {
          newDoc.nivelCo = 'No definido'       
        }
        if(!newDoc.resultCo){
          newDoc.resultCo = 'No definido'
        }
        return newDoc;
      ...
      this.foodList = this.foodList.filter(currentFood => {
        ...
        if (currentFood.name && searchTerm) {            
          return (currentFood.name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 ||
            ...
            currentFood.nivelCo.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 );
        }
      });
    }
    ...
    generatePDF() {
      ...
      this.foodList.forEach(e=>{
        html += '<tr> <td>' + e.name + '</td><td>' + e.apellidos + '</td><td>' + e.cedula + '</td><td>' + e.nivelCo + '</td><td>' + e.resultCo + '</td></tr>'
      })
