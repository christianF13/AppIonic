...
@Component({
  selector: 'app-reportres',
  templateUrl: './reportres.page.html',
  styleUrls: ['./reportres.page.scss'],
})
export class ReportresPage implements OnInit {
  ...
  public foodList: UserRep[];
    ...
    public foodSource$ = this.firestore.collection<UserRep>('users').get().pipe(
      ...
      map(docs =>docs.map(old => {
        const newDoc = old;
        if(!newDoc.nivelResol) {
          newDoc.nivelResol = 'No definido'       
        }
        if(!newDoc.resultResol){
          newDoc.resultResol = 'No definido'
        }
        return newDoc;
      ...
      this.foodList = this.foodList.filter(currentFood => {
        ...
        if (currentFood.name && searchTerm) {            
          return (currentFood.name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 ||
            ...
            currentFood.nivelResol.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 );
        }
      });
    }
    ...
    generatePDF() {
      ...
      this.foodList.forEach(e=>{
        html += '<tr> <td>' + e.name + '</td><td>' + e.apellidos + '</td><td>' + e.cedula + '</td><td>' + e.nivelResol + '</td><td>' + e.resultResol + '</td></tr>'
      })
