...
@Component({
  selector: 'app-reportse',
  templateUrl: './reportse.page.html',
  styleUrls: ['./reportse.page.scss'],
})
export class ReportsePage implements OnInit {
  ...
  public foodList: UserRep[];
    ...
    public foodSource$ = this.firestore.collection<UserRep>('users').get().pipe(
      ...
      map(docs =>docs.map(old => {
        const newDoc = old;
        if(!newDoc.nivelSegu) {
          newDoc.nivelSegu = 'No definido'       
        }
        if(!newDoc.resultSegu){
          newDoc.resultSegu = 'No definido'
        }
        return newDoc;
      ...
      this.foodList = this.foodList.filter(currentFood => {
        ...
        if (currentFood.name && searchTerm) {            
          return (currentFood.name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 ||
            ...
            currentFood.nivelSegu.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 );
        }
      });
    }
    ...
    generatePDF() {
      ...
      this.foodList.forEach(e=>{
        html += '<tr> <td>' + e.name + '</td><td>' + e.apellidos + '</td><td>' + e.cedula + '</td><td>' + e.nivelSegu + '</td><td>' + e.resultSegu + '</td></tr>'
      })
