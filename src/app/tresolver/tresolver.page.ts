...
@Component({
  selector: 'app-tresolver',
  templateUrl: './tresolver.page.html',
  styleUrls: ['./tresolver.page.scss'],
})
export class TresolverPage implements OnInit {
  ...
  public AreaReso: string="Resolución de problemas";
  ...
  ngOnInit() {
    ...
    this.SerPreg.getPreguntasResol().subscribe(preguntas => {
  ...
  getResult(opciones:number[]){
    ...
    this.SerPreg.getPreguntasResol().subscribe(
      (data: any) => {
 ...
 async nivel(){
    ...
    this.SerPreg.area = this.AreaReso;
   ...
   await this.authservice.uploadResultResol(this.SerPreg.correctAnswerCount,this.SerPreg.nivele,this.SerPreg.area );
